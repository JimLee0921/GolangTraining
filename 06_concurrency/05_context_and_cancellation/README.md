...
`context` 用于在多个 goroutine 之间传递取消信号、超时控制和请求相关的上下文信息。
它是 Go 并发编程中的关键组件，能够有效地管理不同任务之间的协作和资源释放。

| 特性                                  | 说明                                 |
|-------------------------------------|------------------------------------|
| **取消传播（Cancellation Propagation）**  | 一处取消可级联影响整个任务树                     |
| **超时/截止时间控制（Timeout/Deadline）**     | 自动取消长时间未完成的任务                      |
| **携带请求范围数据（Request-scoped values）** | 在请求生命周期中传递元数据（如 trace_id, user_id） |
| **线程安全、可并发访问**                      | Context 是并发安全的，只读结构                |

> 底层其实就是对 `done channel` 的封装
