...
func NewHTTPClient(conn net.Conn, opt *Option) (*Client, error) {
	// 向服务器写 CONNECT 请求 等价于在已有的 TCP 连接上写 CONNECT /_geerpc_ HTTP/1.0\n\n
	_, _ = io.WriteString(conn, fmt.Sprintf("CONNECT %s HTTP/1.0\n\n", defaultRPCPath))

	// 只读取一次 HTTP 响应，第二个参数传入一个伪造的 Request{Method: "CONNECT"}，用于解析 Response 时的一些细节
	resp, err := http.ReadResponse(bufio.NewReader(conn), &http.Request{Method: "CONNECT"})
	// 校验状态码，成功则直接切换到 RPC 协议
...
// DialHTTP 带超时版本的 HTTP 版 Dial
func DialHTTP(network, address string, opts ...*Option) (*Client, error) {
	return dialTimeout(NewHTTPClient, network, address, opts...)
}

// XDial 统一入口，支持多种协议格式
// client, err := geerpc.XDial("http@127.0.0.1:9999")
// client, err := geerpc.XDial("tcp@127.0.0.1:8080")
// client, err := geerpc.XDial("unix@/tmp/geerpc.sock")
