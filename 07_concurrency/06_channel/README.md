...
| 问题                                         | 原因           |
|--------------------------------------------|--------------|
| 死锁（fatal error: all goroutines are asleep） | 通道操作阻塞，无协程解除 |
| 关闭后发送                                      | panic        |
| range 未关闭通道                                | 永远阻塞         |
| 多次关闭                                       | panic        |
| 不匹配方向的通道                                   | 编译错误         |
...
| 建议                | 原因               |
|-------------------|------------------|
| 优先使用无缓冲通道实现同步     | 明确顺序与依赖          |
| 使用缓冲通道实现异步任务      | 提升吞吐量            |
| 合理设置缓冲大小          | 避免阻塞或过度积压        |
| 主动关闭通道            | 让接收方安全退出         |
| 使用 `select` 处理多通道 | 避免阻塞             |
| 避免滥用 channel      | 简单同步可用 WaitGroup |
| 禁止共享同一通道作多用途      | 逻辑混乱难以维护         |

---

## 十六、channel 在 CSP 模型中的地位
